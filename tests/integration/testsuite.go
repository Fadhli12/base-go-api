...
`
	...
	// Migration 000027: Idempotency keys
	migration027 := `
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_idempotency_keys_active
...
CREATE INDEX IF NOT EXISTS idx_idempotency_keys_user_id
    ...
    WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_idempotency_keys_organization_id
...
DROP TRIGGER IF EXISTS update_idempotency_keys_updated_at ON idempotency_keys CASCADE;
CREATE TRIGGER update_idempotency_keys_updated_at
    BEFORE UPDATE ON idempotency_keys
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
`
	if err := s.DB.Exec(migration027).Error; err != nil {
		errs = append(errs, fmt.Errorf("migration 000027 (idempotency_keys): %w", err))
	}
